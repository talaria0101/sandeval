...
mkdir -p "$TD/out"
bash "$SWEEP" >/dev/null 2>&1;                     [ $? -eq 2 ] && ok "no --replica -> 2" || bad "no --replica rc"
...
OUT_LOG="$TD/run1.log"
"${GUARD_ENV[@]}" bash "$SWEEP" --replica --baseline "$TD/v1.tsv" "$TD/in" "$TD/out2" >"$TD/run1.log" 2>&1
rc=$?
...
PATH="$TD/shadow:$PATH" "${GUARD_ENV[@]}" bash "$SWEEP" --replica "$TD/in3" "$TD/out2" >"$TD/run127.log" 2>&1
...
"${GUARD_ENV[@]}" bash "$SWEEP" --replica --check "$TD/v1.tsv" "$TD/in" "$TD/out2" >"$TD/run2.log" 2>&1
...
"${GUARD_ENV[@]}" bash "$SWEEP" --replica --expect "$TD/exp.tsv" "$TD/in" "$TD/out2" >"$TD/run3.log" 2>&1
...
"${GUARD_ENV[@]}" bash "$SWEEP" --replica --check "$TD/v1-mut.tsv" "$TD/in" "$TD/out2" >"$TD/run4.log" 2>&1
...
"${GUARD_ENV[@]}" bash "$SWEEP" --replica --expect "$TD/exp6.tsv" --check "$TD/v1-mut.tsv" "$TD/in" "$TD/out2" >"$TD/run5.log" 2>&1
